<?php
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(!$pickup_date || !$return_date) {
            ...
            redirect("../public/book-car.php?car_id=" . $selected_car_id);
        }elseif(!validate_date($pickup_date) || !validate_date($return_date)){
            $_SESSION['error'] = "Invalid date format. Please use YYYY-MM-DD.";
            redirect("../public/book-car.php?car_id=" . $selected_car_id);
        }
        elseif($pickup_date >= $return_date) {
            $_SESSION['error'] = "Pickup date must be before return date.";
            redirect("../public/book-car.php?car_id=" . $selected_car_id);
        }elseif(!$total_price) {
            $_SESSION['error'] = "Invalid price format. Please use a valid price.";
            redirect("../public/book-car.php?car_id=" . $selected_car_id);
        }else{
            try {
                ...
                if ($result) {
                    $_SESSION['register_success'] = 'Booking added successfully!';
                    redirect('../public/book-car.php?car_id=' . $selected_car_id);
                } else {
